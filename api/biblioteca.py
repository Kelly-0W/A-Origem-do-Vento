...
import json
import os
from homebrew_backend import handler as HomebrewHandler
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        if (query.get("acao") or [None])[0] == "homebrew":
            return HomebrewHandler.do_GET(self)
    ...
    def do_OPTIONS(self):
        query = parse_qs(urlparse(self.path).query)
        if (query.get("acao") or [None])[0] == "homebrew":
            return HomebrewHandler.do_OPTIONS(self)
    ...
    def do_POST(self):
        ...
        if (query.get("acao") or [None])[0] != "homebrew":
            ...
            self.send_header("Access-Control-Allow-Origin", "*")
            self.end_headers()
            self.wfile.write(json.dumps({"sucesso": False, "erros": ["Método não permitido."]}, ensure_ascii=False).encode("utf-8"))
            return
        return HomebrewHandler.do_POST(self)
