...
export default function HomebrewCampanha({ campanhaId, usuario }) {
  ...
  return <section className="card-fantasy p-6 mt-8">
    ...
    {erro && <p className="text-blood-bright text-sm mb-3">{erro}</p>}
    {carregando ? <p className="text-mist text-sm">Carregando...</p> : lista.length ? <div className="grid md:grid-cols-2 gap-4">{lista.map(h => <article key={h.id} className="border border-panel-border rounded p-4"><h3 className="text-lg">{h.nome}</h3><p className="text-sm text-mist mt-2">{h.descricao}</p>{h.efeito && <p className="text-sm mt-2">Efeito: {h.efeito}</p>}<div className="text-xs text-mist mt-3">{[h.execucao, h.alcance, h.duracao].filter(Boolean).join(' · ')}{h.custo_arche != null ? ` · ${h.custo_arche} Arché` : ''}</div></article>)}</div> : <p className="text-sm text-mist">Nenhum item aprovado em {label.toLowerCase()}.</p>}
    {dados.eh_mestre ? <div className="mt-8 border-t border-panel-border pt-5"><h3 className="text-xl mb-3">Pedidos aguardando aprovação</h3>{dados.pedidos.length ? <div className="space-y-3">{dados.pedidos.map(p => <article key={p.id} className="border border-panel-border rounded p-4"><strong>{p.conteudo?.nome || 'Criação indisponível'}</strong><p className="text-sm text-mist mt-1">{p.conteudo?.descricao}</p><div className="flex gap-2 mt-3"><button disabled={!!ocupado} onClick={() => responder(p, true)} className="btn-primary text-xs">{ocupado === p.id ? 'Salvando…' : 'Aprovar'}</button><button disabled={!!ocupado} onClick={() => responder(p, false)} className="btn-secondary text-xs">Rejeitar</button></div></article>)}</div> : <p className="text-sm text-mist">Nenhum pedido pendente.</p>}
      <h3 className="text-xl mt-7 mb-3">Criações dos jogadores</h3>{bibliotecaJogadores.length ? <div className="grid md:grid-cols-2 gap-3">{bibliotecaJogadores.map(h => <article key={`${h.owner_uid}_${h.id}`} className="border border-panel-border rounded p-3"><span className="text-[10px] uppercase tracking-widest text-gold">{h.tipo === 'poder_elemental' ? 'Poder elemental' : 'Habilidade de personagem'}</span><h4 className="mt-1">{h.nome}</h4><p className="text-xs text-mist mt-1">{h.descricao}</p></article>)}</div> : <p className="text-sm text-mist">Nenhuma criação dos jogadores nesta categoria.</p>}</div> : <div className="mt-8 border-t border-panel-border pt-5"><h3 className="text-xl mb-3">Seus pedidos</h3>{dados.pedidos.length ? <ul className="space-y-2">{dados.pedidos.map(p => <li key={p.id} className="text-sm">{p.conteudo?.nome || p.skill_id} <span className="text-gold">· {p.status}</span></li>)}</ul> : <p className="text-sm text-mist">Você ainda não enviou pedidos de conteúdo.</p>}</div>}
  </section>
}
